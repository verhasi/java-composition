...
public class Java1_0Validator extends Validators {
    ...
    final Validator noMultiPatternCaseLabels = new SingleNodeTypeValidator<>(SwitchEntry.class, (n, reporter) -> {
        long patternCount = n.getLabels().stream().filter(Expression::isComponentPatternExpr).count();
        if (patternCount > 1) {
            reporter.report(n, new UpgradeJavaMessage("Multiple patterns in case labels not supported.", ParserConfiguration.LanguageLevel.JAVA_22));
        }
    });
